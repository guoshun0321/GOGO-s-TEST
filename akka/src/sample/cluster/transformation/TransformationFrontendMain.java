    ...
    public static void main(String[] args)
    {
        ...
        final Config config =
            ConfigFactory.parseString("akka.remote.netty.tcp.port=" + port)
                .withFallback(ConfigFactory.parseString("akka.cluster.roles = [frontend]")).withFallback(ConfigFactory.load());
        ...
        Cluster.get(system).registerOnMemberUp(new Runnable()
        {
            ...
            public void run()
            {
                system.actorOf(Props.create(MemberUpActor.class), "memberUpActor");
            }
        });
