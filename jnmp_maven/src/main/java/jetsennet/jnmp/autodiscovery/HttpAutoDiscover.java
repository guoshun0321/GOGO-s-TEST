...
import jetsennet.jnmp.autodiscovery.helper.HttpDiscover;

/**
 * SNMP自动发现
 * @author 郭祥
 */
public class HttpAutoDiscover extends AbsAutoDiscover
{
    ...
    public AutoDisResult discover(AutoDisTaskEntity task, int userId, String userName, boolean isLog) throws DiscoverException
    {
        ...
        try
        {
            ...
            HttpDiscover hd = new HttpDiscover();
            hd.setFilter(filter);
            hd.find(coll);

            // 类型
            AbsDiscover cd = new ClassDiscover(10098, "HTTP");
            cd.find(coll);

            // 数据处理
            AbsAutoDisResultHandle handle = new AutoDisResultHandleWithPtl(task.getTaskId(), task.getCollId(), AutoDisConstant.PRO_NAME_HTTP);
