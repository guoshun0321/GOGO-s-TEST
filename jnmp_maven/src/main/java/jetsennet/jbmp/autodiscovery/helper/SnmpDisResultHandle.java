/************************************************************************
日 期：2011-12-02
作 者: 郭祥
版 本：v1.3
描 述: 使用SNMP协议进行自动发现后的结果处理
历 史：
 ************************************************************************/
package jetsennet.jbmp.autodiscovery.helper;

import java.util.ArrayList;
import java.util.List;

import org.apache.log4j.Logger;

import jetsennet.jbmp.autodiscovery.AutoDisConstant;
import jetsennet.jbmp.dataaccess.AutoDisObjDal;
import jetsennet.jbmp.dataaccess.base.ClassWrapper;
import jetsennet.jbmp.entity.AutoDisObjEntity;
import jetsennet.jbmp.util.IPv4AddressUtil;

/**
 * 使用SNMP协议进行自动发现后的结果处理
 * @author 郭祥
 */
public class SnmpDisResultHandle extends AbsAutoDisResultHandle
{

    private AutoDisObjDal adodal;
    private static final Logger logger = Logger.getLogger(SnmpDisResultHandle.class);

    /**
     * @param taskId 参数
     * @param collId 参数
     */
    public SnmpDisResultHandle(int taskId, int collId)
    {
        super(taskId, collId);
        adodal = ClassWrapper.wrapTrans(AutoDisObjDal.class);
    }

    @Override
    public void handle(AutoDisResult result, int userId)
    {
        try
        {
            List<AutoDisObjEntity> oldObjs = adodal.getByTaskId(taskId);
            List<AutoDisObjEntity> newObjs = this.toAutoDisObj(result);
            List<AutoDisObjEntity> retval = this.compare(oldObjs, newObjs);
            adodal.insertOrUpdate(retval);
            this.ins(retval, userId);
        }
        catch (Exception ex)
        {
            logger.error("", ex);
        }
    }

    /**
     * 将采集到的数据转换成AutoDisObjEntity
     * @param result 参数
     * @return 结果
     */
    public List<AutoDisObjEntity> toAutoDisObj(AutoDisResult result)
    {
        List<AutoDisObjEntity> retval = new ArrayList<AutoDisObjEntity>();
        List<SingleResult> irs = result.getIrs();
        for (SingleResult ir : irs)
        {
            // 首先使用ARP协议进行过滤
            ProResult pr = ir.getByPro(AutoDisConstant.PRO_NAME_ARP);
            if (pr == null)
            {
                log.logMiss(ir.getSingleKey());
                continue;
            }
            // 解析xml中的snmp协议信息
            try
            {
                // 通用信息
                AutoDisObjEntity ad = new AutoDisObjEntity();
                String ip = ir.getSingleKey();
                ad.setIp(ip);
                ad.setIpNum(IPv4AddressUtil.stringToLong(ip));

                // SNMP信息
                ProResult snmppr = ir.getByPro(AutoDisConstant.PRO_NAME_SNMP);
                if (snmppr != null)
                {
                    ad.setObjName(snmppr.getByKey(AutoDisConstant.SNMP_SYSNAME));
                    ad.setUserName(snmppr.getByKey(AutoDisConstant.SNMP_COMMUNITY));
                    ad.setPort(snmppr.getByKey(AutoDisConstant.PORT));
                    ad.setVersion(snmppr.getByKey(AutoDisConstant.SNMP_VERSION));
                }
                else
                {
                    ad.setObjName(ip);
                }

                // 分类信息
                ProResult classpr = ir.getByPro(AutoDisConstant.PRO_NAME_CLASS);
                ad.setClassId(Integer.valueOf(classpr.getByKey(AutoDisConstant.CLASS_ID)));
                ad.setCollId(collId);
                ad.setTaskId(taskId);
                ad.setObjStatus(AutoDisObjEntity.STATUS_NEW);
                ad.setObjDesc(ir.toXml());

                retval.add(ad);
            }
            catch (Exception ex)
            {
                logger.error("", ex);
            }
        }
        return retval;
    }
}
