...
import jetsennet.jbmp.protocols.usb.bfss.state.StateMachine;
 ...
 * 采集管理窗口
 */
public class CollMgrSCFrm extends BaseMgrFrm
{
    ...
    private JMenuItem startUsbMenuItem;
    private JMenuItem stopUsbMenuItem;
    private JLabel usbStateLabel;

    private UsbLineState usbState;

    /**
     * @param title
     */
    public CollMgrSCFrm(String title)
    {
        super(title);
        usbState = new UsbLineState(this);
    }
    ...
    protected void initComponents()
    {
            ...
            public void actionPerformed(ActionEvent e)
            {
                ...
                int result =
                    JOptionPane.showOptionDialog(null,
                        "停止集群操作将会同时停止性能采集、Trap接收、Syslog接收和外部接收，是否继续？",
                        "停止集群",
        ...
        // USB管理
        startUsbMenuItem = new JMenuItem();
        //        startUsbMenuItem.setText("启动USB通讯");
        //        startUsbMenuItem.setEnabled(true);
        //        startUsbMenuItem.addActionListener(new java.awt.event.ActionListener()
        //        {
        //            public void actionPerformed(java.awt.event.ActionEvent evt)
        //            {
        //                stratUsb(evt);
        //            }
        //        });
        stopUsbMenuItem = new JMenuItem();
        //        stopUsbMenuItem.setText("停止USB通讯");
        //        stopUsbMenuItem.setEnabled(false);
        //        stopUsbMenuItem.addActionListener(new java.awt.event.ActionListener()
        //        {
        //            public void actionPerformed(java.awt.event.ActionEvent evt)
        //            {
        //                stopUsb(evt);
        //            }
        //        });
        //        JMenu usbMenu = new JMenu();
        //        usbMenu.setText("USB通讯");
        //        usbMenu.add(startUsbMenuItem);
        //        usbMenu.add(stopUsbMenuItem);
        //        addMenu(usbMenu);
        ...
        usbStateLabel = new JLabel(" ");
        this.getContentPane().add(usbStateLabel,
            new GridBagConstraints(0, 1, 0, 1, 1, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
    }

    public void setUsbState(String str)
    {
        usbStateLabel.setText(str);
    }
    ...
    private void stopAlarm(java.awt.event.ActionEvent evt)
    {
        ...
        startExtendMenuItem.setEnabled(false);
        stopExtendMenuItem.setEnabled(false);
        startUsbMenuItem.setEnabled(false);
        stopUsbMenuItem.setEnabled(false);
    }
    ...
    private void startPerf(java.awt.event.ActionEvent evt)
    {
        try
        {
            ...
            {
                logger.info("设置采集方式：通过USB线采集。");
                BFSSCollector.getInstance().start();
            }
            else
            {
                logger.info("设置采集方式：直接采集。");
            }
            usbState.start();
    ...
    private void stopPerf(java.awt.event.ActionEvent evt)
    {
        try
        {
            ...
            if (!collType.equals(DataAgentManager.COLL_TYPE_STR))
            {
                logger.info("准备停止USB模块。");
                BFSSCollector.getInstance().stop();
            }
            usbState.stop();
            startPerfMenuItem.setEnabled(true);
            stopPerfMenuItem.setEnabled(false);
    ...
    public static void main(String[] args)
    {
        java.awt.EventQueue.invokeLater(new Runnable()
        {
            public void run()
            {
                CollMgrSCFrm mgr = new CollMgrSCFrm("采集管理");
                mgr.setVisible(true);
            }
        });
    }
