...
public class Update20131009
{

    /**
     * 日志
     */
    private static final Logger logger = Logger.getLogger(Update20131009.class);
    ...
    public void updateDBDate()
    {
        ...
        try
        {
            ...
            // 更新属性相关信息
            logger.debug("开始更新属性相关的信息。");
            List<AttributeEntity> allAttr = attrDal.getAll();
            int attrUpdateCount = 0;
            for (AttributeEntity attr : allAttr)
            {
                attrUpdateCount++;
                logger.debug("更新属性信息：" + attr.getAttribName() + "； ID：" + attr.getAttribId());
                int alarmId = attr.getAlarmId();
                AlarmEntity alarm = allAlarm.get(alarmId);
                // 存在旧报警规则
            ...
            logger.debug("更新属性结束，总共更新信息：" + attrUpdateCount);
            ...
            List<ObjAttribEntity> allOa = oadal.getAll();
            int oaCount = 0;
    ...
    {
        Update20131009 update = new Update20131009();
        update.update();
    }
