    ...
    public void onReceiveMessage(String msg) throws Exception
    {
        ...
        try
        {
            if (!StringUtil.isNullOrEmpty(msg))
            {
                ...
                Node node = (Node) doc.selectSingleNode("/Msg/ReturnInfo/AlarmSearchPSet");
                String freqNum = ((Element) node).attributeValue("Freq");
                String serviceId = ((Element) node).attributeValue("ServiceID");

                String[] objItem = null;
                if (!StringUtil.isNullOrEmpty(freqNum) && !StringUtil.isNullOrEmpty(serviceId))
                {
                    objItem =
                        sqlExecutor.find(sqlExecutor.getSqlParser().formatCommand(
                            "SELECT OBJ_ID FROM BMP_OBJECT WHERE FIELD_1 LIKE '%" + freqNum + "'" + " and FIELD_2=" + serviceId + " and CLASS_ID=25",
                            new SqlValue[0]));
                }

                if (objItem != null)
                {
                    List list = node.selectNodes("AlarmSearchP");
