...
function fileQueued(file) {
	...
	/*
	 * comment by stephen try { var progress = new FileProgress(file,
	 * this.customSettings.progressTarget); progress.setStatus("Pending...");
...
function fileQueueError(file, errorCode, message) {
	try {
		...
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			jetsennet.alert("You have attempted to queue too many files.\n"
		...
		var errInfo = "选择失败";
		/*
		 * comment by stephen var progress = new FileProgress(file,
		 * this.customSettings.progressTarget); progress.setError();
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	...
	/*
	 * try { var percent = Math.ceil((bytesLoaded / bytesTotal) * 100);
	 * 
	 * var progress = new FileProgress(file,
	 * this.customSettings.progressTarget); progress.setProgress(percent);
	 * progress.setStatus("Uploading..."); } catch (ex) { this.debug(ex); }
	 */
}
...
function uploadSuccess(file, serverData) {
	...
	try {
		if (isSuccess) {
			...
			getAllUnusedMIBFileName();
			var delObject = document.getElementById(file.id + "_del");
			delObject.parentNode.innerHTML = "&nbsp;";
			document.getElementById(this.customSettings.myFileListTargetv + "Count").innerHTML = this.getStats().files_queued;
		} else {
			var tr = document.getElementById(file.id);
			tr.style.color = "red";
			var bar = document.getElementById(file.id + "_bar");
	...
	/*
	 * try { var progress = new FileProgress(file,
	 * this.customSettings.progressTarget); progress.setComplete();
	 * progress.setStatus("Complete."); progress.toggleCancel(false);
	 *  } catch (ex) { this.debug(ex); }
	 */
}
...
function queueComplete(numFilesUploaded) {
	document.getElementById(this.customSettings.myFileListTarget
			+ "SuccessUploadCount").innerHTML = this.getStats().successful_uploads;// add
																					// by
																					// stephen
	/*
	 * comment by stephen var status = document.getElementById("divStatus");
