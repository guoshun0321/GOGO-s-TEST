...
//	var allSystem = hasAllSystemRights();
    ...
    condition.SqlConditions.push(JetsenWeb.SqlCondition.create("GROUP_TYPE","1,6",JetsenWeb.SqlLogicType.And,JetsenWeb.SqlRelationType.In,JetsenWeb.SqlParamType.Numeric));
...
//自定义 在loadTableObject方法的基础上修改的，加了权限过滤
function getCollName(controlId,noEmpty,loadTable,loadTextField,loadValueField)
{
    var control = $(controlId);
    control.options.length = 0;
    if(!noEmpty)
    ...
    var sqlQuery = new JetsenWeb.SqlQuery();   
    var queryTable = JetsenWeb.createQueryTable(loadTable,"t");
    queryTable.addJoinTable(JetsenWeb.createJoinTable("BMP_OBJGROUP", "b","t.coll_id = b.num_val1", JetsenWeb.TableJoinType.LEFT));     
    sqlQuery.GroupFields = "t.COLL_ID,t.COLL_NAME"; 
    JetsenWeb.extend(sqlQuery,{IsPageResult:0,KeyId: "COLL_ID",PageInfo:null,QueryTable:queryTable,ResultFields:"t."+loadTextField+",t."+loadValueField});
...
{
	var condition = new JetsenWeb.SqlConditionCollection();
	condition.SqlConditions.push(JetsenWeb.SqlCondition.create("CLASS_ID",7,JetsenWeb.SqlLogicType.And,JetsenWeb.SqlRelationType.Equal,JetsenWeb.SqlParamType.Numeric));
	var gSqlQuery = new JetsenWeb.SqlQuery();
	var gQueryTable = JetsenWeb.createQueryTable("BMP_OBJECT","");
	JetsenWeb.extend(gSqlQuery,{KeyId:"OBJ_ID",QueryTable:gQueryTable,Conditions:condition,ResultFields:"OBJ_ID,OBJ_NAME"});
	var ws = new JetsenWeb.Service(DMP_SYSTEM_SERVICE);
	ws.soapheader = JetsenWeb.Application.authenticationHeader;
	ws.async = false;
	ws.oncallback = function(ret)
	{
		...
		if (records)
		{
			...
			for(var i = 0; i < length; i++)
			{
				var tsStreamInfo = records[i];
				selectElement.options.add(new Option(tsStreamInfo["OBJ_NAME"], tsStreamInfo["OBJ_ID"]));
			}
		}
	};
	ws.onerror = function(ex){ jetsennet.error(ex + "！");};
...
{
	var gSqlQuery = new JetsenWeb.SqlQuery();
	var gQueryTable = JetsenWeb.createQueryTable("NMP_CTRLWORD","");
	var condition = new JetsenWeb.SqlConditionCollection();
	condition.SqlConditions.push(JetsenWeb.SqlCondition.create("CW_TYPE",type,JetsenWeb.SqlLogicType.And,JetsenWeb.SqlRelationType.Equal,JetsenWeb.SqlParamType.Numeric));
	JetsenWeb.extend(gSqlQuery,{KeyId:"CW_ID",QueryTable:gQueryTable,Conditions:condition,ResultFields:"CW_ID,CW_NAME"});
	var ws = new JetsenWeb.Service(DMP_SYSTEM_SERVICE);
...
//是否为非法开始时间和结束时间
function illegalityTime(startTime, endTime){
...
//是否为非法开始日期和结束日期
function illegalityDate(startTime, endTime){
	if(startTime>endTime){
...
//根据复选框的选择状态改变按钮状态
function changeButtonStateByCheckboxState(checkboxName, buttonIds)
{
	var chks = JetsenWeb.Form.getCheckedValues(checkboxName);
	var buttonIdList = buttonIds.split(",");
	for (var i = 0; i < buttonIdList.length; i++)
	{
		var button = $(buttonIdList[i]);
		if (button)
		{
			button.disabled = chks.length == 0;
			button.className = chks.length == 0 ? "disablebutton" : "button";
		}
	}
}