...
//通过编辑实例化自动发现对象
function newObjectElement(obj_id, coll_id) {
...
function getCollector() {
	...
	JetsenWeb.extend(sqlQuery, { KeyId: "a.COLL_ID", QueryTable: queryTable, ResultFields: "a.*" });
...
function setEquipmentField() {
	...
	$("txt_USERNAME").value = valueOf(equipment, "USER_NAME");
...
function addObjectToGroup() {
	var checkObjIds = JetsenWeb.Form.getCheckedValues("chkAllObject");
    ...
    JetsenWeb.extend(dialog, { submitBox: true, cancelBox: true, windowStyle: 1, maximizeBox: true, minimizeBox: true, size: { width: 500, height: dialogHeight }, title: "选择对象组" });
...
function getObjAttribute(currentObjId, attrTypes, classId) {
	...
	if(attrTypes)
	{
		queryConditoin.SqlConditions.push(JetsenWeb.SqlCondition.create("A.ATTRIB_TYPE", attrTypes, JetsenWeb.SqlLogicType.And, JetsenWeb.SqlRelationType.In, JetsenWeb.SqlParamType.Numeric,true));
	}
	else
	{
		queryConditoin.SqlConditions.push(JetsenWeb.SqlCondition.create("A.ATTRIB_TYPE", "104,105,107", JetsenWeb.SqlLogicType.And, JetsenWeb.SqlRelationType.In, JetsenWeb.SqlParamType.Numeric,true));
	}
	...
	JetsenWeb.extend(sqlQuery, { KeyId: "A.OBJATTR_ID", QueryTable: queryTable, ResultFields: "A.*,C.ALARM_ID,C.ALARM_NAME" });
    ...
    ws.oncallback = function (ret) {
    	switch(attrTypes)
		{
			...
			case "101,106":
		        ...
		        var gGridList = new JetsenWeb.UI.GridList();
		        gGridList.bind($("insConfigDiv"), $("configattributeList"));
		        //gGridList.calcSize();
				break;
			case "102":
				 //获取监控信息
			...
			case "105":
				...
				var gGridList = new JetsenWeb.UI.GridList();
		        gGridList.bind($("insTrapDiv"), $("signalattributeList"));
				break;
			case "107":
				var retObj = JetsenWeb.Xml.toObject(ret.resultVal, "Record");
		        if(retObj != null){
				...
				var gGridList = new JetsenWeb.UI.GridList();
		        gGridList.bind($("insTrapDiv"), $("syslogattributeList"));
				break;
			case "104,105,107":
		        ...
		        var gGridList = new JetsenWeb.UI.GridList();
		        gGridList.bind($("insTrapDiv"), $("otherattributeList"));
				break;
			case "":
				$('insTrapDiv').innerHTML = "";
				$('insTrapDiv').innerHTML = JetsenWeb.Xml.transformXML("xslt/otherAttributelist.xslt", ret.resultVal);
...
function listAttrib(attribTypes){
	...
	if(attribTypes == '101,106' && calssId != '')
	{
		...
		if(curAttrib != null && curAttrib != ""){
			condition.SqlConditions.push(JetsenWeb.SqlCondition.create("A.ATTRIB_ID",curAttrib,JetsenWeb.SqlLogicType.And,JetsenWeb.SqlRelationType.NotIn,JetsenWeb.SqlParamType.Numeric,true));
		}
...
function showAttribList(attribTypes) {
    ...
    dialog.onsubmit = function () {
        ...
        ws.async = false;
        ws.oncallback = function (ret) {
    		$('instanceResult').innerHTML = JetsenWeb.Xml.transformXML("xslt/attribInsresult.xslt", ret.resultVal);
    		var gGridList = new JetsenWeb.UI.GridList();
            gGridList.bind($("instanceResult"), $("InstanceResultTable"));
...
function showOtherAttribList()
{
	...
	if(type)
	{
		...
		var dialog = new JetsenWeb.UI.Window("show-ConfigAttrib-win");
	    JetsenWeb.extend(dialog, { submitBox: true, cancelBox: true, windowStyle: 1, maximizeBox: true, minimizeBox: true, size: { width: 700, height: dialogHeight }, title: "第1步:选择属性" });
	    dialog.controls = ["listAttribByType"];
	    dialog.onsubmit = function () {
	    	var checkAttribId = JetsenWeb.Form.getCheckedValues("selectAttribute");
	    	if(checkAttribId.length <= 0) 
	    	{
...
{
	var oldFileForm = $("importFileForm");
	oldFileForm.reset();//情况上次文件选择框的内容
	setImportCollId();
	setImportClassType();
	var dialog = new JetsenWeb.UI.Window("show-selectFile-win");
	JetsenWeb.extend(dialog, { submitBox: true, cancelBox: true, windowStyle: 1, maximizeBox: false, minimizeBox: false, size: { width: 400, height: 180 }, title: "选择文件" });
	dialog.controls = ["divSelectFile"];
	dialog.onsubmit = function () {
		var selectImportColl = getSelectedValue($("import_coll"));
		var selectImportClassId = getSelectedValue($("import_classid"));
		var selectImportClassType;
		if(selectImportClassId)
		{
			for(var i=0; i<arrAttibClass.length; i++) {
				var eachOne = arrAttibClass[i];
				if(eachOne["CLASS_ID"] == selectImportClassId) {
					selectImportClassType = eachOne["CLASS_TYPE"];
					break;
				}
			}
		}
    	...
    	if(path.length == 0)
    	{
    		jetsennet.alert("请选择需要导入的文件！");
    		return;
    	}
    	var index = path.lastIndexOf(".");
    	var expression = path.substring(index + 1);
    	if(expression != "xls")
    	{
    		jetsennet.alert("文件类型不正确！");
    		return;
    	}
    	getGroupIdByCollId(selectImportColl);//根据采集器ID获得采集组ID，为后台实例化对象用
    	if(!collectorGroupId)
    	{
    		jetsennet.alert("无法获得采集组！");
    		return;
    	}
    	var fileName = new Date().getTime() + "." + expression;
    	var form = $("importFileForm");
    	form.action = "../../servlet/BMPFileImportAndCheckServlet?fileName=" + fileName + "&uploadPath=attachment/importObject/" + "&coll_id=" + selectImportColl + "&collgroup_id=" + collectorGroupId + "&create_user=" + encodeURI(encodeURI(JetsenWeb.Application.userInfo.UserName)) + "&user_id=" + JetsenWeb.Application.userInfo.UserId + "&class_id=" + selectImportClassId + "&class_type=" + selectImportClassType;
    	form.submit();
    	JetsenWeb.UI.Windows.close("show-selectFile-win");
    	var frameImportDocument = $("frameImport").contentWindow.document;
    	frameImportDocument.body.innerHTML = "";
    	var dialog2 = new JetsenWeb.UI.Window("result-object-win");
    	JetsenWeb.extend(dialog2, { windowStyle: 1, maximizeBox: false, minimizeBox: false, size: { width: 350, height: 100 }, title: "批量导入" });
    	dialog2.controls = ["divImportTip"];
    	importIntervalId = setInterval(function()
    	{
    		var importResult = $("frameImport").contentWindow.document.body.innerHTML;
    		if(importResult != "")
    		{
    			JetsenWeb.UI.Windows.close("result-object-win");
    			if(importResult.indexOf("成功") != -1)
    			{
    				showObjList(selectImportClassType, selectImportClassId);
    			}
    			else if(importResult.indexOf("1") != -1)
    			{
    				importResult = "文件格式不对！";
    				jetsennet.alert(importResult);
    			}
    			else if(importResult.indexOf("2") != -1)
    			{
    				importResult = "文件格式不对或执行导入文件出错！";
    				jetsennet.alert(importResult);
    			}
...
function setImportCollId()
{
	var import_coll = $("import_coll");
	import_coll.options.length = 0;
	import_coll.options.add(new Option("请选择", ""));
	if(arrColltor != null && arrColltor.length > 0) {
		for(var i=0; i<arrColltor.length; i++) {
			var obj = arrColltor[i];
			import_coll.options.add(new Option(obj["COLL_NAME"], obj["COLL_ID"]));
		}
	}
}

function setImportClassType()
{
	var import_classid = $("import_classid");
	import_classid.options.length = 0;
	import_classid.options.add(new Option("请选择", ""));
	for(var i=0; i<arrAttibClass.length; i++) {
		var obj = arrAttibClass[i];
		import_classid.options.add(new Option(obj["CLASS_NAME"], obj["CLASS_ID"]));
	}
}
